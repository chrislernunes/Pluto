...
class PBSARHL(EventInterfacePositional):
    ...
    def on_bar_complete(self):
        ...
        # self.futprice.loc[len(self.futprice.index)] = ([self.current_tick['ts'], self.current_tick['o'], self.current_tick['h'], self.current_tick['l'], self.current_tick['c'], self.current_tick['v'], self.current_tick['oi']])
            ...
            if self.position_ce == 1:
                ...
                if self.to_exit:
                    self.success_ce, _ = self.place_trade(
                        self.now, 'BUY', self.lot_size, self.symbol_ce
                        ,price=self.get_tick(self.now, self.symbol_ce).get('c') if self.now.time() == datetime.time(9, 15) else self.get_tick(self.now, self.symbol_ce).get('c'),
            ...
            if self.position_pe == 1:
                ...
                if self.to_exit:
                    self.success_pe, _ = self.place_trade(
                        self.now, 'BUY', self.lot_size, self.symbol_pe,
                        price=self.get_tick(self.now, self.symbol_pe).get('c') if self.now.time() == datetime.time(9, 15) else self.get_tick(self.now, self.symbol_pe).get('c'),
        ...
        if (self.now.time() >= self.start_time and self.now.time() < datetime.time(15, 27) ) :   #or (self.now.time() == datetime.time(9, 7))
            if self.position_ce == 0 and self.signal == -1 :
                ...
                if self.symbol_ce is not None:
                    self.symbol_ce_hedge = self.find_symbol_by_premium(self.now, self.underlying, expiry_idx, "CE", hedge_prices_dict[self.underlying])
                    # # HEDGE SANITY CHECK
                    price = self.get_tick(self.now, self.symbol_ce_hedge)['c']
                    if np.isnan(price) or price == 0:
                        self.symbol_ce_hedge = self.find_symbol_by_premium(self.now, self.underlying, expiry_idx, "CE", 10)
                    # self.symbol_ce_hedge = None
                    # sell CE
                    self.success_ce, self.entry_price_ce = self.place_trade(
                        self.now, 'SELL', self.lot_size, self.symbol_ce,
                        price=self.get_tick(self.now, self.symbol_ce).get('c') if self.now.time() == datetime.time(9, 15) else self.get_tick(self.now, self.symbol_ce).get('c'),
            ...
            if self.position_pe == 0 and self.signal == 1 :
                ...
                if self.symbol_pe is not None:
                    self.symbol_pe_hedge = self.find_symbol_by_premium(self.now,self.underlying, expiry_idx, "PE", hedge_prices_dict[self.underlying])
                    # # HEDGE SANITY CHECK
                    print(self.symbol_pe_hedge)
                    price = self.get_tick(self.now, self.symbol_pe_hedge)['c']
                    if np.isnan(price) or price == 0:
                        self.symbol_pe_hedge = self.find_symbol_by_premium(self.now, self.underlying, expiry_idx, "PE", 10)                    
                    # self.symbol_pe_hedge = None
                    # sell PE
                    print(self.symbol_pe)
                    print(self.symbol_pe_hedge)
                    print(self.now)
                    self.success_pe, self.entry_price_pe= self.place_trade(
                        self.now, 'SELL', self.lot_size, self.symbol_pe,
                         price=self.get_tick(self.now, self.symbol_pe).get('c') if self.now.time() == datetime.time(9, 15) else self.get_tick(self.now, self.symbol_pe).get('c'),
                        note='ENTRY'
                    )
                    if self.success_pe:
