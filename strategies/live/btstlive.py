...
class BTSTDIRV2(EventInterfacePositional):
    ...
    def on_bar_complete(self):
        ...
        if self.strat_type == 'r' and self.roll_or_no in [True, 'True']: 
        ...
        elif self.strat_type == 'r' and self.roll_or_no in [False, 'False']: 
        ...
        if self.position_ce == 1:
            ...
            # SL
            if self.now.time() >= datetime.time(9,25) and self.now.time() <= self.stop_time:
        ...
        if self.roll_or_no in [True, 'True']:
            # ROLL-OVER ENTRY
